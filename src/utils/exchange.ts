...
export const BASE_FEE = new Percent(JSBI.BigInt(25).toString(), BIPS_BASE);
...
export function calculateSlippageAmount(
  ...
  return [
    JSBI.divide(
      JSBI.multiply(
        JSBI.BigInt(value.quotient.toString()), 
        JSBI.BigInt(10000 - Number(slippage))
      ),
      JSBI.BigInt(BIPS_BASE)
    ),
    JSBI.divide(
      JSBI.multiply(
        JSBI.BigInt(value.quotient.toString()), 
...
export function basisPointsToPercent(num: number): Percent {
  return new Percent(JSBI.BigInt(num).toString(), BIPS_BASE);
}
