...
export function useDerivedBurnInfo(
  ...
  const liquidityValueA =
    ...
    JSBI.greaterThanOrEqual(
      JSBI.BigInt(totalSupply.quotient.toString()),
      JSBI.BigInt(userLiquidity.quotient.toString())
    )
  ...
  const parsedAmounts: {
    ...
    [Field?.CURRENCY_A]: !zapMode
      ...
      ? CurrencyAmount.fromRawAmount(
        ...
        BigInt(
            JSBI.add(
                JSBI.BigInt(percentToRemove.multiply((liquidityValueA as any)?.quotient).quotient.toString()),
                JSBI.BigInt(estimateZapOutAmount.quotient.toString())
            ).toString()
        )
    )
      : !removalCheckedA
